...
def _type_id_from_requirements(requirements: dict) -> str | None:
    if not isinstance(requirements, dict):
        return None

    def normalized(k: str) -> str:
        return _normalize_text(str(k))

    def normalized_v(v: str) -> str:
        return _normalize_text(str(v))

    for key, value in requirements.items():
        key_norm = normalized(key)
        val_norm = normalized_v(value)
        if "тип" in key_norm:
            if "hdd" in val_norm or "жесткий" in val_norm or "винчестер" in val_norm:
                return "storage.hdd"
            if "ssd" in val_norm or "твердотел" in val_norm:
                return "storage.ssd"

    all_vals = " ".join(normalized_v(v) for v in requirements.values() if v is not None)
    if "hdd" in all_vals or "жесткий" in all_vals or "винчестер" in all_vals:
        return "storage.hdd"
    if "ssd" in all_vals or "твердотел" in all_vals:
        return "storage.ssd"
    return None
...
def normalize_products(parsed: dict) -> dict:
    ...
    for p in products:
        ...
        if term:
            ...
            p.get("technical_requirements") or {}
            ...
            type_from_requirements = _type_id_from_requirements(p.get("technical_requirements") or {})
            if type_from_requirements:
                p["type_id"] = type_from_requirements
                p["type_score"] = 100
            elif _has_indicator(["ssd"]):
